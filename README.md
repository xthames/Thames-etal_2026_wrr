[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.18148549.svg)](https://doi.org/10.5281/zenodo.18148549)
...
1. Create an empty directory and populate it with the hierarchy included in `workflow/`. Both `workflow/configs/` and `workflow/scripts/` are already filled; these contain configuration files or scripts relevant to execute the rest of our experiment. Consider the `workflow/` directory -- where all other directories are accessible -- as the root directory for the purposes of this experiment
...
This will create the ~2.5TB worth of output files that are used to reproduce our figures (*note that you will need the corresponding available memory to do so*).
