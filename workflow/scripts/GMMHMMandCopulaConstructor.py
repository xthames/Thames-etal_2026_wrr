...
def ConstructCopulae(pTData, hmmYears, genScenarios=False):
    ...
    # -- stationarity
    if not genScenarios:
        InvestigateStationarity(pTDict)
...
def InvestigateStationarity(ptDict):
    ...
    for weather_var in ["PRECIP", "TEMP"]:
        ...
        for m, month in enumerate(months):
            resids = ptDict[month][weather_var + " ARFit"].resid
        ...
        plt.tight_layout()
        stationarity_fig.savefig(plotsDir + r"/copulae/{}/{}_Stationarity_{}.svg".format(dataRepo, repoName, weatherVar))
        plt.close()
