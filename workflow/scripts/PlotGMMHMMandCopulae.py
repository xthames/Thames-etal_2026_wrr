...
# plot the radial plots of copula monthly metrics
def PlotCopulaAICs():
    ptDict = CopulaDict 
    metrics_fig = plt.figure(figsize=(16, 9))
    metrics_fig.suptitle('Monthly Copula Fitness Metrics')
    months, metrics = list(ptDict.keys()), ["AIC", "S_n", "T_n"]
    for a in range(len(metrics)):
        ...
        for j, month in enumerate(months):
            fit_info = ptDict[month]["CopulaDF"]
    ...
    plt.tight_layout()
    metrics_fig.savefig(plotsDir + r"/copulae/{}/{}_CopulaFitMetricPlots.svg".format(dataRepo, repoName))
    plt.close()
